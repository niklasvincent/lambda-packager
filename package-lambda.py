...
class Git(object):
    ...
    def getChecksum(directory):
        ...
        stdout, stderr = Git.executeInDirectory(directory, ['git', 'show-ref'])
    ...
    def getUncommitedChanges(directory):
        ...
        stdout, stderr = Git.executeInDirectory(directory, ['git', 'status', '-s'])
        return stdout.split('\n')
    ...
    def deduplicateFileList(file_list):
        ...
        for file in file_list:
            if file.relativePath.endswith(".pyc"):
                uncompiled_file = File(file.absolutePath[:-1], file.relativePath[:-1])
...
def files_to_package(source_directory, library_directories = list(), exclude = list()):
    ...
    if library_directories:
        library_directories = [os.path.abspath(d) for d in library_directories]
...
def package(strict, source_directory, library_directories = list(), exclude = list()):
    """Package Lambda from sources and libraries"""
    if strict:
        enforce_strict(source_directory)
    files = files_to_package(source_directory, library_directories, exclude)
    output_filename = zip_filename(source_directory)
